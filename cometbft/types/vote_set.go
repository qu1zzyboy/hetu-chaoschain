...
func (voteSet *VoteSet) addVerifiedVote(
	...
	if quorum <= votesByBlock.sum {
		...
		for _, vote := range votesByBlock.votes {
			...
			if voteCodePower[vote.VoteCode] >= quorum {
				// Only consider the first quorum reached
				if voteSet.maj23 == nil {
					voteSet.maj23 = &Maj23Vote{
...
func (voteSet *VoteSet) TwoThirdsMajority() (blockID BlockID, ok bool, code int64) {
	if voteSet == nil {
	...
	defer voteSet.mtx.Unlock()
	if voteSet.maj23 != nil {
		return *voteSet.maj23.BlockID, true, voteSet.maj23.VoteCode
	}
	return BlockID{}, false, 0
}
...
func (voteSet *VoteSet) MakeExtendedCommit(ap ABCIParams) *ExtendedCommit {
	...
	for i, v := range voteSet.votes {
		...
		if sig.BlockIDFlag == BlockIDFlagCommit && !v.BlockID.Equals(*voteSet.maj23.BlockID) {
...
type blockVotes struct {
	...
	votes     []*VoteWithPower // valIndex -> *Vote
	sum       int64            // vote sum
}
...
func newBlockVotes(peerMaj23 bool, numValidators int) *blockVotes {
	return &blockVotes{
		...
		votes:     make([]*VoteWithPower, numValidators),
		sum:       0,
	}
}
...
func (vs *blockVotes) addVerifiedVote(vote *Vote, votingPower int64) {
	...
	if existing := vs.votes[valIndex]; existing == nil {
		...
		vs.votes[valIndex] = &VoteWithPower{
			Vote:  vote,
			Power: votingPower,
		}
		vs.sum += votingPower
...
func (vs *blockVotes) getByIndex(index int32) *Vote {
	...
	if vs.votes[index] == nil {
		return nil
	}
	return vs.votes[index].Vote
}
