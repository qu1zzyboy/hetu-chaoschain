...
OUTER_LOOP:
	for {
		...
		// Maybe send Height/Round/Prevotes
		{
			...
			prs := ps.GetRoundState()
			if rs.Height == prs.Height {
				if maj23, ok, _ := rs.Votes.Prevotes(prs.Round).TwoThirdsMajority(); ok {
		...
		// Maybe send Height/Round/Precommits
		{
			...
			prs := ps.GetRoundState()
			if rs.Height == prs.Height {
				if maj23, ok, _ := rs.Votes.Precommits(prs.Round).TwoThirdsMajority(); ok {
					peer.TrySend(p2p.Envelope{
						ChannelID: StateChannel,
						Message: &cmtcons.VoteSetMaj23{
							Height:  prs.Height,
		...
		// Maybe send Height/Round/ProposalPOL
		{
			...
			if rs.Height == prs.Height && prs.ProposalPOLRound >= 0 {
				if maj23, ok, _ := rs.Votes.Prevotes(prs.ProposalPOLRound).TwoThirdsMajority(); ok {
